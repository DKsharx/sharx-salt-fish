...
from ..ocr.OcrHelper import OcrApi
from ..screenshot.ScreenshotUtil import ScreenshotUtil
...
class Automation:
    ...
    def init_automation(self):
        ...
        self.mouse_click_random = Input.mouse_click_random
    ...
    def take_screenshot(self, crop=(0, 0, 0, 0)):
        result = ScreenshotUtil.take_screenshot(self.window_title, crop=crop)
        if result:
    ...
    def find_image_element(self, target, threshold, scale_range):
        try:
            ...
            logger.debug(("目标图片：{target} 相似度：{max_val}").format(target=target, max_val=max_val))
            ...
            logger.error(("寻找图片出错：{e}").format(e=e))
        return None, None
    ...
    def find_image_and_count(self, target, threshold, pixel_bgr):
        try:
            ...
            logger.error(("寻找图片并计数出错：{e}").format(e=e))
            return None
    ...
    def find_text_element(self, target, include, need_ocr=True, relative=False):
        ...
        try:
            if need_ocr:
                self.ocr_result = OcrHelper.recognize_multi_lines(np.array(self.screenshot))
            if not self.ocr_result:
                logger.debug(("目标文字：{target} 未找到，没有识别出任何文字").format(target=", ".join(target)))
                return None, None
            for box in self.ocr_result:
                ...
                if ((include is None or not include) and text in target) or (include and any(t in text for t in target)):
                    ...
                    logger.debug(("目标文字：{target} 相似度：{max_val}").format(target=self.matched_text, max_val=box[1][1]))
                    if relative == False:
                    ...
                    return top_left, bottom_right
            logger.debug(("目标文字：{target} 未找到，没有识别出匹配文字").format(target=", ".join(target)))
            return None, None
        except Exception as e:
            logger.error(("寻找文字：{target} 出错：{e}").format(target=", ".join(target), e=e))
            return None, None

    def find_min_distance_text_element(self, target, source, include, need_ocr=True):
        if need_ocr:
            ocr = OcrApi()
            self.ocr_result = ocr.recognize_multi_lines(self.screenshot)
        if not self.ocr_result:
            logger.debug(("目标文字：{target} 未找到，没有识别出任何文字").format(target=f"{target}, {source}"))
            return None, None
        ...
        for box in self.ocr_result:
            ...
            if ((include is None or not include) and source == text) or (include and source in text):
                logger.debug(("目标文字：{target} 相似度：{max_val}").format(target=source, max_val=box["score"]))
                source_pos = box["box"]
                break
        if source_pos is None:
            logger.debug(("目标文字：{target} 未找到，没有识别出匹配文字").format(target=source))
            return None, None
        ...
        for box in self.ocr_result:
            ...
            if ((include is None or not include) and target == text) or (include and target in text):
                ...
                logger.debug(("目标文字：{target} 相似度：{max_val} 距离：{min_distance}").format(target=target, max_val=box[1][1], min_distance=distance))
                if distance < min_distance:
                    min_distance = distance
                    target_pos = pos
        if target_pos is None:
            logger.debug(("目标文字：{target} 未找到，没有识别出匹配文字").format(target=target))
            return None, None
        logger.debug(("目标文字：{target} 最短距离：{min_distance}").format(target=target, min_distance=min_distance))
    ...
    def click_element(self, target, find_type, threshold=None, max_retries=1, crop=(0, 0, 0, 0), take_screenshot=True, relative=False, scale_range=None, include=None, need_ocr=True, source=None, offset=(0, 0)):
        ...
        return False
    def random_click_half_place(self):
        region =  ScreenshotUtil.get_window_region_by_title(self.window_title)
        tmpList = list(region)
        tmpList[0] = tmpList[0]*2
        tmpList[1] = tmpList[1]*2
        self.mouse_click_random( tmpList[0], tmpList[1], tmpList[2], tmpList[3])
        return True
    def get_single_line_text(self, crop=(0, 0, 0, 0), blacklist=None, max_retries=3):
        for i in range(max_retries):
            ...
            logger.debug(("ocr识别结果: {ocr_result}").format(ocr_result=ocr_result))
            if ocr_result:
                return ocr_result[0]
        return None
